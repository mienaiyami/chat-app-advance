...
io.use(async (socket, next) => {
	try {
		const session = await getTokenExpress(socket.request as unknown as Request);
			...
			if (conversation) {
				...
				for (const memberId of memberIds) {
					if (memberId !== userId) {
						const memberSocketIds = onlineUsers.get(memberId);
						if (memberSocketIds) {
							io.to(Array.from(memberSocketIds)).emit("conversation:read", {
								conversationId: params.conversationId,
								userId,
								timestamp,
							});
			...
			if (conversation) {
				...
				for (const memberId of memberIds) {
					if (memberId !== userId) {
						const memberSocketIds = onlineUsers.get(memberId);
						if (memberSocketIds) {
							io.to(Array.from(memberSocketIds)).emit("user:typing", {
								conversationId,
								userId,
							});
						}
			...
			if (conversation) {
				...
				for (const memberId of memberIds) {
					if (memberId !== userId) {
						const memberSocketIds = onlineUsers.get(memberId);
						if (memberSocketIds) {
							io.to(Array.from(memberSocketIds)).emit("user:stop_typing", {
								conversationId,
								userId,
							});
						}
