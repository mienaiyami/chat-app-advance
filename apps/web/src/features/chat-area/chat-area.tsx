...
export function ChatArea() {
	...
	const { sendMessage, editMessage, isSending, isUploadingFile, handleTyping } =
		useMessage();
	...
	const chatOpened = conversations.find((c) => c.id === activeConversationId) as
	...
	if (!chatOpened) {
		return (
			<div className="grid h-full max-h-screen flex-1 select-none place-items-center rounded-r-lg border border-l-0">
	...
	return (
		<div className="flex h-full max-h-screen flex-1 flex-col rounded-r-lg border border-l-0">
			{isLoadingMembers ? (
				<ChatHeaderSkeleton />
			) : (
				<ChatHeader
