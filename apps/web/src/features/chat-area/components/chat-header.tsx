			...
			<div>
				<DropdownMenu>
					...
					<DropdownMenuContent align="end">
						{chatOpened.type === "direct" && (
							<>
								...
								{chatOpened.name?.includes("(Unknown)") && (
									<DropdownMenuItem
										onClick={() => {
											const otherMember = chatOpened.members.find(
												(m) => m.userId !== currentUser?.id,
											);
											if (otherMember) {
												handleUpdateContact(otherMember.userId, "add");
											}
										}}
									>
										Add Contact
						...
						{chatOpened.type === "group" && (
							<>
								<DropdownMenuItem onClick={() => setLeaveGroupDialogOpen(true)}>
									Leave Group
								</DropdownMenuItem>
								<DropdownMenuItem
									onClick={() => setGroupDetailsDialogOpen(true)}
								>
									Group Details
								</DropdownMenuItem>
							</>
						...
						<DropdownMenuItem
							onClick={() => {
								if (chatOpened) {
									handleUpdateMutedChat(chatOpened.id, !isChatMuted);
								}
							}}
						>
				...
				>
					<DialogContent>
						<DialogHeader>
							...
							<DialogDescription>
								Are you sure you want to leave this group? Only admins can
								re-add you.
							</DialogDescription>
						</DialogHeader>
						<DialogFooter>
