...
export function GroupDetailsDialog({
	...
	if (!conversationDetails || conversationDetails.type !== "group") return null;
	...
	return (
		<DialogContent className="cursor-default sm:max-w-[425px]">
			...
			<div className="grid gap-2 py-4">
				<div className="flex items-center gap-4">
					<Avatar className="h-20 w-20">
						...
						/>
						<AvatarFallback>
							{(conversationDetails.name || "Group").slice(0, 2).toUpperCase()}
						</AvatarFallback>
					</Avatar>
					<div>
						<h2 className="font-bold text-2xl">{conversationDetails.name}</h2>
				...
				<p className="flex items-center text-muted-foreground text-sm">
					...
					Created on: {formatDate(new Date(conversationDetails.createdAt))}
				</p>
					...
					<div className="mt-2 space-y-2">
						...
						{isLoadingJoinLink ? (
							...
							isAdmin ? (
								...
								<p className="text-muted-foreground text-sm">
									No invite link available. Ask an admin to create one.
								</p>
							)
						) : (
							<div className="space-y-2">
								...
								<p className="text-muted-foreground text-xs">
									Expires: {formatDate(new Date(joinLink.expiresAt))}
								</p>
								{isAdmin && (
									<Button
										size="sm"
				...
				<div className="select-none">
					<h3 className="relative mb-2 font-semibold text-lg">Members</h3>
					...
					<ScrollArea className="h-[200px]">
						{filteredMembers.map((member) => (
							<div
								key={member.id}
								className="flex h-12 items-center justify-between rounded-md p-2 hover:bg-accent/50"
							>
